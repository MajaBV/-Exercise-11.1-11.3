...
for score_dict in best_score:
    print("Dan/sat {0} Igrac {1} je imao/la {2} pokušaja. Skriveni broje je broj {3}".format(score_dict.get("datum"), score_dict.get("igrac"), score_dict.get("broj pokusaja"), score_dict.get("tajni broj")))
...
while True:
    ...
    if guess == secret:
        best_score.append({"broj pokusaja": broj_pokusaja, "datum": str(datetime.datetime.now()), "igrac": player_name, "tajni broj": secret})
